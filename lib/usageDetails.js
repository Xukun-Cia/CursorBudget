...
function buildTooltipLines(data) {
  ...
  if (gpt.ok) {
    if (gpt.stale) {
      lines.push(tooltipRow('GPT状态', `沿用最近有效值 · ${gpt.error || '本轮刷新失败'}`));
    }
    const windows = Array.isArray(gpt.windows) ? gpt.windows : [];
    if (windows.length) {
      windows.forEach((row, index) => {
        const reset = row.resetAt ? formatDateTime(row.resetAt) : '—';
        const plan = index === 0 && gpt.plan ? `${gpt.plan} · ` : '';
        lines.push(tooltipRow(row.label, `${plan}${fmtGptPct(row.percent)} · 重置 ${reset}`));
      });
    } else {
      const reset = gpt.resetAt ? formatDateTime(gpt.resetAt) : '—';
      lines.push(tooltipRow('GPT', `${gpt.plan || '—'} · ${fmtGptPct(gpt.percent)} · 重置 ${reset}`));
    }
...
/** GPT quota values currently advance in whole percentage points. Keep any
 * future fractional precision the service returns without printing fake .00. */
function fmtGptPct(v) {
  if (v === null || v === undefined || !Number.isFinite(v)) return '—';
