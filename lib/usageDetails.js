...
function parseUsageSummaryRich(summary) {
  ...
  const base = {
    ...
    includedLimitCents: plan?.limit ?? null,
    includedUsedCents: plan?.used ?? null,
    includedBonusCents: plan?.breakdown?.bonus ?? null,
    // Same accounting as planUsage.totalSpend (included + bonus).
    totalSpendCents: plan?.breakdown?.total ?? null,
...
function shouldSkipUsageEvent(event) {
  return SKIPPED_USAGE_EVENT_KINDS.has(event?.kind);
}
...
function buildTooltipLines(data) {
  ...
  } else if (s?.autoPercentUsed != null) {
    lines.push(tooltipRow('Auto', fmtPct(s.autoPercentUsed)));
  }

  if (s?.includedLimitCents != null) {
  ...
  if (s?.billingCycleStart && s?.billingCycleEnd) {
    ...
    const resetLabel = data.apiBudget?.resetAt
      ? formatDateTime(data.apiBudget.resetAt)
      : data.resetDate || '—';
