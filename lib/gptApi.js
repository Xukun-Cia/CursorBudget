...
function gptError(message, transient, statusCode) {
  const err = new Error(message);
  err.transient = Boolean(transient);
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            try {
              ...
              reject(gptError('GPT 用量响应不是 JSON', true, res.statusCode));
            }
            return;
          }
          if (res.statusCode === 401 || res.statusCode === 403) {
            reject(gptError('GPT 登录已过期，请打开 GPT App 再登录一次', false, res.statusCode));
            return;
          }
          const transient = res.statusCode === 408 || res.statusCode === 429 || res.statusCode >= 500;
          reject(gptError(`GPT 用量 HTTP ${res.statusCode}`, transient, res.statusCode));
        });
      },
    );
    req.on('error', (err) => reject(gptError(err.message || String(err), true)));
    req.setTimeout(20000, () => {
      req.destroy();
      reject(gptError('GPT 用量请求超时', true));
    });
    req.end();
  });
}
...
function emptyGpt(error, transient) {
  return {
    ok: false,
...
async function fetchGptPublic() {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      requestError = err;
      if (!err.transient || attempt === 1) break;
      await delay(TRANSIENT_RETRY_DELAY_MS);
    }
  }
  if (requestError) {
    const failed = emptyGpt(requestError.message || String(requestError), requestError.transient);
  ...
  if (!main || !Number.isFinite(main.percent)) {
    const failed = emptyGpt('GPT 用量响应缺少可用额度窗口', true);
    failed.plan = parsed.plan || auth.plan;
    failed.cycleStart = auth.cycleStart;
    failed.cycleEnd = auth.cycleEnd;
    return failed;
  }
